...
$profile_layout   = empty($dokan_appearance['store_header_template']) ? 'layout2' : $dokan_appearance['store_header_template'];
$store_address    = dokan_get_seller_short_address($store_user->get_id(), false);

$dokan_store_time_enabled = isset($store_info['dokan_store_time_enabled']) ? $store_info['dokan_store_time_enabled'] : '';
$store_open_notice        = isset($store_info['dokan_store_open_notice']) && !empty($store_info['dokan_store_open_notice']) ? $store_info['dokan_store_open_notice'] : __('Store Open', 'postero');
$store_closed_notice      = isset($store_info['dokan_store_close_notice']) && !empty($store_info['dokan_store_close_notice']) ? $store_info['dokan_store_close_notice'] : __('Store Closed', 'postero');
                    ...
                    <div class="profile-info">
                        ...
                        <?php if (!empty($store_rating['count'])): ?>
                            <div class="dokan-seller-rating" title="<?php echo sprintf(esc_attr__('Rated %s out of 5', 'postero'), esc_attr($store_rating['rating'])) ?>">
                                ...
                                <p class="rating">
                                    <?php echo esc_html(sprintf(__('%s out of 5', 'postero'), $store_rating['rating'])); ?>
                                </p>
                            </div>
                        <?php endif ?>
                        ...
                        <ul class="dokan-store-info">
                            ...
                            <?php if ($show_store_open_close == 'on' && $dokan_store_time_enabled == 'yes') : ?>
                                <li class="dokan-store-open-close">
                                    <i class="postero-icon-store-2"></i>
                                    <div class="store-open-close-notice">
                                        ...
                                        <span class="postero-icon-angle-down"></span>
                                        <div id="vendor-store-times">
                                            <div class="store-times-heading">
                                                <i class="fas fa-calendar-day"></i>
                                                <h4><?php esc_html_e('Weekly Store Timing', 'postero'); ?></h4>
                                            </div>
                                            <?php
                                            foreach (dokan_get_translated_days() as $day_key => $day) :
                                                ...
                                                <div class="store-time-tags">
                                                    <?php $class_store = $today === $day_key ? 'current_day' : ''?>
                                                    <div class="store-days <?php echo esc_attr($class_store); ?>"><?php echo esc_html($day); ?></div>
                                                    <div class="store-times">
                                                        <?php if ($store_status === 'close') : ?>
                                                            <span class="store-close"><?php esc_html_e('CLOSED', 'postero'); ?></span>
                                                        <?php endif; ?>
