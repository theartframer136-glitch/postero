<div id="dokan-seller-listing-wrap" class="grid-view">
    <div class="seller-listing-content">
        <?php if ($sellers['users']) : ?>
                                ...
                                <div class="store-data-container">
                                    <div class="featured-favourite">
                                        <?php if ($is_store_featured) : ?>
                                            <div class="featured-label"><?php esc_html_e('Featured', 'postero'); ?></div>
                                        <?php endif ?>
                                    ...
                                    <?php if ('on' === $show_store_open_close && 'yes' === $dokan_store_time_enabled) : ?>
                                        <?php if (dokan_is_store_open($seller->ID)) { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-open-status" title="<?php esc_attr_e('Store is Open', 'postero'); ?>"><?php esc_html_e('Open', 'postero'); ?></span>
                                        <?php } else { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-closed-status" title="<?php esc_attr_e('Store is Closed', 'postero'); ?>"><?php esc_html_e('Closed', 'postero'); ?></span>
                                        <?php } ?>
                                    <?php endif ?>
                                    ...
                                    <div class="store-data <?php echo esc_attr($store_open_is_on); ?>">
                                        ...
                                        <?php if (!dokan_is_vendor_info_hidden('address') && $store_address): ?>
                                            ...
                                            ?>
                                            <div class="store-address"><?php echo wp_kses($store_address, $allowed_tags); ?></div>
                                        <?php endif ?>
            ...
            if ($num_of_pages > 1) {
                ...
                $pagination_args = array(
                    ...
                    'type'      => 'array',
                    'prev_text' => '<i class="postero-icon-angle-left"></i><span>' . __('Previous', 'postero') . '</span>',
                    'next_text' => '<span>' . __('Next', 'postero') . '</span><i class="postero-icon-angle-right"></i>',
                );
                ...
                if ($page_links) {
                    ...
                    printf('%s',$pagination_links); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
                }
        ...
        <?php else: ?>
            <p class="dokan-error"><?php esc_html_e('No vendor found!', 'postero'); ?></p>
        <?php endif; ?>
    </div>
</div>
