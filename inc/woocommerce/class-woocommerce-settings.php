...
if (!class_exists('Postero_WooCommerce_Settings')) :

    /**
     * The Postero WooCommerce Settings Class
     */
    class Postero_WooCommerce_Settings {
        ...
        public function settings_product_tabs($tabs) {
            ...
            $tabs['sizechart'] = array(
                'label'    => esc_html__('Postero settings', 'postero'),
                'target'   => 'postero_options',
                'class'    => array(),
                'priority' => 80,
            );
        ...
        public function settings_options_product_tab_content() {
            ...
            ?>
            <div id='postero_options' class='panel woocommerce_options_panel'>
                <div class='options_group'><?php
                    ...
                    $options[''] = esc_html__('Select size chart', 'postero');
                    ...
                    woocommerce_wp_select(array(
                        ...
                        'label'   => esc_html__('Size chart', 'postero'),
                        'options' => $options,
                        'value'   => $value,
                    ));

                    ?>
                    <p class="form-field form-field-wide wc-customer-custom"><?php echo esc_html__('Size chart will take templates name prefix is "SizeChart"', 'postero'); ?></p>

                    <?php
                    woocommerce_wp_text_input(array(
                        'id'    => '_video_select',
                        'label' => esc_html__('Url Video', 'postero'),
                    ));
                    woocommerce_wp_textarea_input(array(
                        'id'    => '_extra_info',
                        'label' => esc_html__('Extra info', 'postero'),
                    ));
        ...
        public function render_sizechart_button() {
            ...
            $chart_id = $this->check_chart($slug);
            if (!empty($slug) && $chart_id) {
                echo '<a href="#" class="sizechart-button">' . esc_html__('Size Guide', 'postero') . '</a>';
            }
        }
        ...
        public function render_sizechart_template() {
            ...
            $chart_id = $this->check_chart($slug);
            if (!empty($slug) && $chart_id) {
                echo '<div class="sizechart-popup"><a href="#" class="sizechart-close"><i class="postero-icon-times-circle"></i></a>' . Elementor\Plugin::instance()->frontend->get_builder_content($chart_id) . '</div><div class="sizechart-overlay"></div>';
            }
        }
    }

    return new Postero_WooCommerce_Settings();
