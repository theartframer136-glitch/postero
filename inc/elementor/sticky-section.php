...
class Postero_Sticky_Module {
    ...
    public function scripts() {
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/elementor-sticky.js', ['jquery'], POSTERO_VERSION, true);
        wp_enqueue_script('postero-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], POSTERO_VERSION, true);
    }
    ...
    public function register_controls(Element_Base $element) {
        $element->add_control(
            'sticky',
            [
                'label'              => esc_html__('Sticky', 'postero'),
                'type'               => Controls_Manager::SELECT,
                'options'            => [
                    ''       => esc_html__('None', 'postero'),
                    'top'    => esc_html__('Top', 'postero'),
                    'bottom' => esc_html__('Bottom', 'postero'),
                ],
                'separator'          => 'before',
                'render_type'        => 'none',
        ...
        foreach ($active_devices as $device) {
            $label                          = 'desktop' === $device ? esc_html__('Desktop', 'postero') : $active_breakpoint_instances[$device]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label'              => esc_html__('Sticky On', 'postero'),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label'              => esc_html__('Offset', 'postero'),
        ...
        $element->add_responsive_control(
            ...
            [
                'label'              => esc_html__('Effects Offset', 'postero'),
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'min'                => 0,
                'max'                => 1000,
        ...
        if ($this->is_instance_of($element, $types)) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label'              => esc_html__('Stay In Column', 'postero'),
    ...
    public function register_controls_search_popup(Element_Base $element) {
        ...
        $element->add_control(
            'search_popup_parent',
            [
                'label'        => esc_html__('Parent search popup', 'postero'),
                'type'         => Controls_Manager::SWITCHER,
