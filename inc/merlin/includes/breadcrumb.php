...
class Postero_breadcrumb {
    ...
    private function __construct() {
        ...
        add_filter('postero_page_title', [$this, 'hide_title']);
    ...
    public function breadcrumb_register() {
        $labels = [
            'name'               => esc_html__('Breadcrumbs', 'postero'),
            'singular_name'      => esc_html__('Breadcrumb', 'postero'),
            'menu_name'          => esc_html__('Breadcrumbs', 'postero'),
            'name_admin_bar'     => esc_html__('Breadcrumb', 'postero'),
            'add_new'            => esc_html__('Add New', 'postero'),
            'add_new_item'       => esc_html__('Add New Breadcrumb', 'postero'),
            'new_item'           => esc_html__('New Header Footer & Blocks Template', 'postero'),
            'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'postero'),
            'view_item'          => esc_html__('View Header Footer & Blocks Template', 'postero'),
            'all_items'          => esc_html__('All Breadcrumb', 'postero'),
            'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'postero'),
            'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'postero'),
            'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'postero'),
            'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'postero'),
        ];
        ...
        register_post_type('postero-breadcrumb', $args);
    }
    ...
    public function register_admin_menu() {
        add_submenu_page(
            'themes.php',
            esc_html__('Breadcrumbs', 'postero'),
            esc_html__('Breadcrumbs', 'postero'),
            'edit_pages',
            'edit.php?post_type=postero-breadcrumb'
        );
    }
    ...
    function ehf_register_metabox() {
        add_meta_box(
            'ehf-meta-box',
            esc_html__('Breadcrumb Options', 'postero'),
            [
                $this,
                'efh_metabox_render',
            ],
        ...
        <table class="hfe-options-table widefat">
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Display On', 'postero'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should appear.', 'postero'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    ...
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Display Rules', 'postero'),
                            'value'          => '[{"type":"basic-global","specific":null}]',
                            'tags'           => 'site,enable,target,pages',
                            'rule_type'      => 'display',
                            'add_rule_label' => esc_html__('Add Display Rule', 'postero'),
                        ],
                        $include_locations
                    );
                    ?>
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Do Not Display On', 'postero'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'postero'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Exclude On', 'postero'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add Exclusion Rule', 'postero'),
                            'rule_type'      => 'exclude',
                        ],
                        $exclude_locations
                    );
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('User Roles', 'postero'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'postero'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Users', 'postero'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add User Rule', 'postero'),
                        ],
                        $users
                    );
                    ?>
    ...
    public static function get_template_id() {
        ...
        $hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('postero-breadcrumb', $option);

        foreach ($hfe_templates as $template) {
            return apply_filters('postero_breadcrumb_id', $template['id']);
        }
    ...
    public function block_template_frontend() {
        if (is_singular('postero-breadcrumb') && !current_user_can('edit_posts')) {
    ...
    function load_canvas_template($single_template) {
        ...
        if ('postero-breadcrumb' == $post->post_type) {
